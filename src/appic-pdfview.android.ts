...
export class AppicPdfView extends AppicPdfViewCommon {
  ...
  public onLoaded(): void {
    ...
    this.loadPDF(this.value_src).catch((err) => {
      ...
      AppicPdfViewCommon.notifyOfEvent(
        AppicPdfViewCommon.errorEvent,
        pdfViewRef
      );
    });
  }
  ...
  public loadPDF(src: string): Promise<any> {
    ...
    return Promise.resolve()
      .then(() => {
        console.log("AppicPdfView Android (Step 1) Clear cache: " + src);
        return this.tempFolder.clear();
      })
      .then(() => {
        console.log("AppicPdfView Android (Step 2) Download: " + src);
      ...
      .then(src => {
        console.log("AppicPdfView Android (Step 3) Download ok: " + src);
        if (src.indexOf("~") >= 0) {
        ...
        console.log("AppicPdfView Android (Step 4) Resolved local path: " + src);
      ...
      let onLoadHandler = (() => {
        ...
        return new pdfviewer.listener.OnLoadCompleteListener({
          loadComplete: numPages => {
            console.log("AppicPdfView Android (Step 5) Pages loaded: " + numPages);

            setTimeout(() => {
              if (that.value_bookmark_label) {
                console.log(
                  "AppicPdfView Android (Step 6) Go to outlinelabel: ",
                  src,
                ...
                console.log(
                  "AppicPdfView Android (Step 6) Go to outlinepath: ",
                  src,
